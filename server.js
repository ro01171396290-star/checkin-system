...
initDB().then(() => {
  ...
  // Batch update settings (for designer backend)
  app.post('/api/admin/settings-batch', adminMiddleware, async (req, res) => {
    ...
    for (const [key, value] of Object.entries(settings)) {
      ...
      const exists = await getRow('SELECT key FROM settings WHERE key = ?', [key]);
      if (exists) {
        await exec("UPDATE settings SET value = ?, updated_at = datetime('now') WHERE key = ?", [String(value), key]);
      } else {
        await exec('INSERT INTO settings (key, value) VALUES (?, ?)', [key, String(value)]);
      }
    }
  ...
  app.listen(PORT, () => {
    ...
    console.log(`  Checkin:  http://localhost:${PORT}/checkin_normal_final.html`);
    console.log(`  Admin:    http://localhost:${PORT}/admin.html`);
    console.log(`  Config:   http://localhost:${PORT}/admin_config.html`);
    console.log(`  Designer: http://localhost:${PORT}/admin_designer.html`);
